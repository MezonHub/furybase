...
import (
	...
	fdexKeeper "github.com/furyunderverse/furybase/x/fdex/keeper"
...
func TestCalPoolUnit(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			poolUnits, lpunits := fdexKeeper.CalPoolUnit(
				tc.oldPoolUnits,
				tc.nativeAssetBalance,
				tc.externalAssetBalance,
				tc.nativeAssetAmount,
...
func TestCalcSwapResult(t *testing.T) {
	...
	for _, tc := range testcases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			y, fee := fdexKeeper.CalSwapResult(tc.Y, tc.X, tc.x, tc.inputIsBase)
...
func TestCalRemoveAmount(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			if tc.panicErr != "" {
				require.PanicsWithError(t, tc.panicErr, func() {
					fdexKeeper.CalRemoveAmount(tc.poolUnits, tc.rmUnits, tc.swapUnits, tc.nativeAssetBalance, tc.externalAssetBalance, tc.inputIsBase)
				})
				return
			}

			baseTokenAmount, tokenAMount, swapAmount := fdexKeeper.CalRemoveAmount(tc.poolUnits, tc.rmUnits, tc.swapUnits, tc.nativeAssetBalance, tc.externalAssetBalance, tc.inputIsBase)
