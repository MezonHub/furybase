...
func TestMsgServerCreatePoolSuccess(t *testing.T) {
	srv, fdexKeeper, ctx, sdkCtx := setupMsgServer(t)
	...
	swapPool, found := fdexKeeper.GetSwapPool(sdkCtx, lpDenom)
...
func TestMsgServerAddRmProviderShouldWork(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, fdexKeeper.HasProvider(sdkCtx, creator2))
	// rm provider
	...
	require.NoError(t, err)
	require.False(t, fdexKeeper.HasProvider(sdkCtx, creator2))
}
...
func TestMsgServerAddRmPoolCreatorShouldWork(t *testing.T) {
	...
	require.NoError(t, err)
	require.True(t, fdexKeeper.HasPoolCreator(sdkCtx, creator2))
	// rm provider
	...
	require.NoError(t, err)
	require.False(t, fdexKeeper.HasPoolCreator(sdkCtx, creator2))
}
...
func TestMsgServerToggleSwitchShouldWork(t *testing.T) {
	...
	require.True(t, fdexKeeper.GetProviderSwitch(sdkCtx))
	...
	require.NoError(t, err)
	require.False(t, fdexKeeper.GetProviderSwitch(sdkCtx))
}
...
func TestMsgServerAddLiquiditySuccess(t *testing.T) {
	...
	_, err = srv.AddProvider(ctx, &msgAddProvider)
	require.NoError(t, err)
	require.True(t, fdexKeeper.HasProvider(sdkCtx, provider))
